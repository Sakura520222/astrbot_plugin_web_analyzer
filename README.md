...
**当前版本 v1.4.4 - v1.4.7 正在进行重大代码重构，目前版本还不稳定和完善，请谨慎使用！**

### 🚨 当前状态
- ⚠️ **重构进行中** - 代码结构正在调整，可能存在未知问题
- ⚠️ **功能测试** - 所有功能需要进一步测试验证
- ⚠️ **稳定性待提升** - 建议在生产环境使用前充分测试
...
### 💡 使用建议
- 🧪 **测试环境优先** - 建议先在测试环境中验证
- 📢 **反馈问题** - 如遇问题请及时提交 Issue
- ⏳ **等待稳定版** - 生产环境建议等待后续稳定版本
- 🔄 **定期更新** - 关注项目更新，及时获取修复和改进

**提示：重构完成后，插件的代码质量、可维护性和扩展性都将得到显著提升！**
...
#### ✨ 功能增强
- **优化配置结构**
- **新增截图临时文件管理器** - 实现专用的 `ScreenshotTempManager` 模块，使用 TTL 机制管理截图临时文件
- **LRU 缓存策略** - 截图内存缓存采用 LRU 算法，自动管理缓存容量
- **后台自动清理** - 新增后台任务自动清理过期的临时文件
- **并发控制机制** - 添加信号量控制截图和消息处理的并发数量
- **批量截图准备** - 支持批量准备截图临时文件路径

#### 🐛 Bug修复
- **临时文件清理优化** - 修复临时文件可能过早被删除的问题
- **并发安全改进** - 通过 TTL 机制避免并发场景下的文件访问冲突

#### 📊 性能优化
- **内存使用优化** - LRU 缓存自动清理最久未使用的截图
