...
- **修复 httpx 代理参数废弃** - `httpx>=0.24.0` 中 `proxies` 参数已废弃，改为 `proxy`，修复配置代理后 TypeError 崩溃
...
- **修复 Playwright 浏览器路径检测缺失** (`core/analyzer`)
  - `_check_browser_installed_async` 的 `possible_exec_paths` 补充 `chrome-linux64/chrome` 和 `chrome-headless-shell-linux64/chrome-headless-shell` 路径（新版 Playwright 使用 `chrome-linux64` 替代 `chrome-linux`）
...
- **修复 httpx.Limits 参数错误** - `httpx.Limits` 不支持 `max_response_size` 参数导致 TypeError 崩溃，改为通过 `Content-Length` 响应头检查响应大小
- **修复新版平台适配器消息发送兼容性** - 优先使用 bot API 发送消息以支持自动撤回，不可用时自动回退到 `event.send()`

### [v1.5.8] - 2026-04-14

#### 🔒 安全修复

- **修复 SSRF 漏洞** - 限制 URL 协议仅为 http/https，检测并阻止私有/回环 IP 地址访问
- **限制 HTTP 响应体大小** - 最大 10MB，防止超大响应导致内存溢出
- **代理配置日志脱敏** - 隐藏代理认证信息，防止凭据泄露
- **修复字符串格式化注入** - 对用户可控内容进行花括号转义，防止 format 异常
- **缓存文件命名升级** - 从 MD5 升级为 SHA-256，提升安全性

#### 🔄 重构

- **截图配置结构重构** - 将截图宽高（`screenshot_width`/`screenshot_height`）合并为 `截图尺寸` 嵌套对象

#### 🐛 Bug修复

- 优化浏览器池事件循环处理逻辑
- 移除 ScreenshotTempManager 中潜在的异步冲突任务

### [v1.5.7] - 2026-04-12

#### ✨ 功能增强

- **新增网页抓取模式配置** - 新增 `fetch_mode` 配置项，支持 `httpx`（默认）和 `playwright` 两种模式，Playwright 模式可抓取 JS 动态页面，失败时自动回退
- **新增 LLMTOOL 多 URL 处理策略** - 新增 `llmtool_url_strategy` 配置项，支持 `auto_analyze`、`llm_hint`、`batch_tool` 三种策略

#### 🐛 Bug修复

- 优化浏览器实例回收异常处理，避免回收失败导致程序中断
- 增强 `llmtool_url_strategy` 配置校验，无效值自动回退
- 修复错误处理结果缺少 `has_screenshot` 字段的问题
...
#### ✨ 功能增强
- **新增LLM事件传播控制** - 新增 `allow_llm_propagation` 配置项，启用后自动分析URL不会阻止消息继续传递给LLM，用户会同时收到分析结果和LLM回复

### [v1.5.4] - 2026-03-29

#### ✨ 功能增强

- **新增截图隐藏IP功能** - 启用后截图浏览器将通过代理访问目标网页，防止真实IP泄露
  - 新增 `hide_ip` 配置项（默认关闭，需配合代理使用）
  - 代理配置同时应用于 Playwright 浏览器实例（此前仅应用于网页抓取请求）
  - 双层 WebRTC 屏蔽机制，防止通过 WebRTC 泄漏真实 IP
