...
- **新增配置 API 选项值白名单校验** - `select` 类型字段（如 `fetch_mode`、`sandbox_mode`、`analysis_mode` 等）仅接受 schema 中定义的合法选项，防止通过 API 提交非法值

#### 🎨 代码重构

- Schema 字段映射改用完整路径作为键，消除不同层级同名字段的潜在键冲突
- 新增 Schema JSON 缓存机制，避免每次 API 调用重复读取磁盘
- 移除 `_validate_value_range` 未使用的参数，收窄 `_safe_float` 返回类型
- 重命名 `_cleanup_legacy_cache_files` → `_cleanup_stale_cache_files`，准确反映清理范围
...
- **管理面板新增配置编辑功能** - 配置页面从只读视图升级为完整的可编辑表单
  - 根据配置项类型自动渲染对应输入控件（开关、数字框、下拉选择、文本框等）
  - 每个配置子分组提供独立的「保存」和「重置」按钮
  - 修改未保存的配置项显示视觉标记
  - 新增 `GET /dashboard/config/schema` 和 `POST /dashboard/config/update` API 端点
...
- **新增浏览器管理命令** `/web_browser`（别名：`/浏览器管理`、`/网页浏览器`，仅管理员可用）
  - 查看浏览器安装和运行状态（安装路径、安装时间、目录大小、实例池数量等）
  - 卸载浏览器（关闭所有实例、删除安装目录、清理安装状态）
- 改进浏览器安装路径查询，查询状态时不再自动创建目录

#### 🐛 Bug修复

- 修复浏览器检查标记作用域错误，多个 `WebAnalyzer` 实例不再重复执行浏览器检测
...
- 消除 `--no-sandbox` 和 `--disable-setuid-sandbox` 的硬编码，非容器环境默认启用 Chromium 安全沙箱
