...
- **修复页面导航超时硬编码** (`core/analyzer`)
  - 将 `page.goto()` 的硬编码超时 `60000ms` 改为使用实例配置 `self.timeout * 1000`，使页面加载超时与用户配置的超时时间一致
  - 此前无论用户如何配置超时时间，页面导航始终使用 60 秒固定值
...
#### ✨ 功能增强
- **新增LLM事件传播控制** (`main.py`, `core/config_loader`, `_conf_schema.json`)
  - 新增 `allow_llm_propagation` 配置项（消息管理 → 事件传播，默认开启）
  - 启用后，插件自动分析完URL不会阻止消息继续传递给LLM，用户会同时收到分析结果和LLM回复
  - 修复自动分析模式下事件被拦截导致LLM无法收到原始消息的问题
...
#### ✨ 功能增强
- **新增截图隐藏IP功能** (`core/analyzer`, `main.py`, `core/config_loader`)
  - 新增 `hide_ip` 配置项，启用后截图浏览器将通过代理访问目标网页
  - 代理配置同时应用于 Playwright 浏览器实例（此前仅应用于 httpx 抓取请求）
  - 双层 WebRTC 屏蔽机制：Chromium 启动参数 + 页面级 JS 注入，防止通过 WebRTC 泄漏真实 IP
  - 错误回退浏览器启动同样支持 IP 隐藏配置，确保一致性

#### ⚙️ 配置调整
- 新增 `hide_ip` 配置项（网络设置分类，默认关闭），需配合代理使用
...
- **修复路径拼接错误** - 解决 `unsupported operand type(s) for /: 'str' and 'str'` 错误
  ...
  - 修复 `core/analyzer.py` 中的 `_get_browser_install_path()` 和浏览器状态文件路径初始化
  - 添加类型检查和详细的调试日志
...
- **新增持久化方法** - `_load_install_status()` 和 `_save_install_status()` 管理安装状态
...
#### ✨ 功能增强
- **新增截图临时文件管理器** - 实现专用的 `ScreenshotTempManager` 模块，使用 TTL 机制管理截图临时文件的生命周期
- **LRU 缓存策略** - 截图内存缓存采用 LRU（最近最少使用）算法，自动管理缓存容量
- **后台自动清理** - 新增后台任务自动清理过期的临时文件，优化磁盘空间使用
- **并发控制机制** - 添加信号量控制截图和消息处理的并发数量，避免资源耗尽
- **批量截图准备** - 支持批量准备截图临时文件路径，提高多链接处理效率

#### 🎯 代码质量提升
- **模块职责分离** - 将截图临时文件管理逻辑从 `MessageHandler` 中分离到独立的 `ScreenshotTempManager` 类
- **配置加载器重构** - 优化 `core/config_loader.py` 的代码结构和格式
- **配置架构更新** - 重新组织 `_conf_schema.json` 配置结构，提高可读性和可维护性
- **代码注释完善** - 为新增功能添加详细的文档字符串和注释

#### 🐛 Bug修复
- **临时文件清理优化** - 修复临时文件可能过早被删除的问题，确保文件在使用期间不被误删
- **并发安全改进** - 通过 TTL 机制避免并发场景下的文件访问冲突
- **缓存数据一致性** - 优化内存缓存和文件缓存的同步机制

#### 📊 性能优化
- **内存使用优化** - LRU 缓存自动清理最久未使用的截图，避免内存占用过高
- **I/O 性能提升** - 优先使用内存缓存，减少磁盘 I/O 操作
- **并发处理优化** - 通过信号量控制并发数量，避免系统资源过载
