...
| 方法 | 说明 |
|------|------|
| `await self.put_kv_data(key, value)` | 写入键值对 |
| `await self.get_kv_data(key, default)` | 读取键值，可指定默认值 |
| `await self.delete_kv_data(key)` | 删除键值对 |
...
# self.name 为插件名称，在 v4.9.2 及以上版本可用
# 低于此版本请自行指定插件名称
```
